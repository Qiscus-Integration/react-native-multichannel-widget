...
import {isEmpty, qiscusLogger} from '../utils';
...
export const MultichannelWidgetProvider = (props) => {
  ...
  let baseURLMultichannel = 'https://multichannel.qiscus.com';
  ...
  const setup = (appId, options) => {
    ...
    Qiscus.init(appId, baseURL, mqttURL, brokerLbURL, uploadURL);
  };
      ...
      setTimeout(() => {
        Qiscus.qiscus.getNonce()
          .then(async ({nonce}) => {
            ...
            let config = {
              method: 'post',
              url: `${baseURLMultichannel}/api/v1/qiscus/initiate_chat`,
              headers: {
