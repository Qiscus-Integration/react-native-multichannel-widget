...
import {FlatList, Keyboard, Clipboard} from 'react-native';
...
const MultichannelWidget = (props) => {
  ...
  const onOpenActionMessage = (item) => {
    if (item.type !== MessageType.TEXT &&
      ...
      item.type !== MessageType.ATTACHMENT) return;
  ...
  const sendMessage = () => {
    ...
    let comment = isReply
      ...
      : Qiscus.qiscus.generateMessage({ roomId: Qiscus.qiscus.selected.id, text: newMessage });

    Keyboard.dismiss();

    if (type == '' && !isEmpty(newMessage)) {
      if (newMessage.length > 400) {
        qiscusToast('Pesan lebih dari 400 karater.');
        return;
      }
    } else {
      if (newMessage.length > 400) {
  ...
  const _onPressSendAttachment = () => {
    ...
    onPressSendAttachment().then(opts => {
      ...
      if (!(sizeInMB <= 5)) {
        qiscusToast('File size lebih dari 5MB');
        return;
      }
